...
JWT_UID = "4122233548"
JWT_PW = "046BE245EDAFF9905E48D8FDED4E31AD87A7AA82F57699AC17E93AD9634DBA0D"
TELEGRAM_TOKEN = "8146195055:AAHXb11E9Urxy6Rpr27RZKVCTN0vzE9ObUE" 
DEFAULT_OWNERS = [5905311134]  # قائمة المالكين الافتراضية
...
def send_json_response(chat_id, message, success=True, data=None, command=None, reply_to_message_id=None):
    """إرسال رد JSON مع معالجة الأخطاء"""
    try:
        ...
        bot.send_message(chat_id, "❌ حدث خطأ في معالجة طلبك", reply_to_message_id=reply_to_message_id)
...
def is_owner(user_id):
    """التحقق إذا كان المستخدم من المالكين"""
    return user_id in OWNERS

def is_allowed_user(user_id):
    """التحقق إذا كان المستخدم مسجلاً وله نقاط متبقية"""
    users = load_data(USERS_FILE)
...
/add &lt;ID&gt; &lt;أيام&gt; - إضافة صديق مع مدة محددة
/rm &lt;ID&gt; - حذف صديق
/show - عرض الأصدقاء
...
"""
    ...
    send_json_response(message.chat.id, "قائمة الأوامر", True, response_data, "help", message.message_id)
    log_action("HELP_REQUESTED", message.from_user.id)

@bot.message_handler(commands=['owner'])
def owner_commands(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="owner", reply_to_message_id=message.message_id)
        return
    response_data = {
        "commands": [
            {"command": "/addowner [user_id]", "description": "إضافة مالك جديد"},
            {"command": "/removeowner [user_id]", "description": "إزالة مالك"},
            {"command": "/listowners", "description": "عرض قائمة المالكين"},
            {"command": "/adduser [user_id] [points]", "description": "إضافة مستخدم جديد"},
            {"command": "/allow [group_id]", "description": "تفعيل البوت في مجموعة"},
            {"command": "/delete_all", "description": "حذف جميع الأصدقاء"},
            {"command": "/bio [نص]", "description": "تغيير البايو"},
            {"command": "/stats", "description": "عرض إحصائيات البوت"}
        ]
    }
    
    try:
        send_json_response(message.chat.id, "أوامر المالكين", True, response_data, "owner", message.message_id)
    except Exception as e:
...
def allow_group(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="allow", reply_to_message_id=message.message_id)
        return
    ...
    try:
        ...
        send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /allow <group_id>", False, command="allow", reply_to_message_id=message.message_id)

@bot.message_handler(commands=['adduser'])
def add_user_command(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="adduser", reply_to_message_id=message.message_id)
        return
    ...
    try:
        ...
        send_json_response(message.chat.id, "تمت إضافة المستخدم بنجاح", True, response_data, "adduser", message.message_id)
    except Exception as e:
        send_json_response(message.chat.id, f"خطأ: {str(e)}\nاستخدم: /adduser <user_id> <points>", False, command="adduser", reply_to_message_id=message.message_id)
...
def add_friend(message):
    ...
    if not is_allowed_user(message.from_user.id) and not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="add", reply_to_message_id=message.message_id)
        return
        
    try:
        parts = message.text.split()
        if len(parts) < 2:
            send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /add <ID> <أيام>", False, command="add", reply_to_message_id=message.message_id)
            return
        ...
        if not uid.isdigit():
            send_json_response(message.chat.id, "يجب أن يكون ID رقمًا صحيحًا!", False, command="add", reply_to_message_id=message.message_id)
            return
            
        if not is_owner(message.from_user.id) and not use_point(message.from_user.id):
            send_json_response(message.chat.id, "ليس لديك نقاط كافية لإضافة أصدقاء!", False, command="add", reply_to_message_id=message.message_id)
            return
        ...
        if days:
            ...
            if expire_time:
                ...
                send_json_response(message.chat.id, "تم إرسال طلب إضافة الصديق بنجاح", True, response_data, "add", message.message_id)
            else:
                send_json_response(message.chat.id, "فشل في إضافة الصديق", False, command="add", reply_to_message_id=message.message_id)
        else:
            Add_FrEind(uid)
            response_data = {
                "uid": uid,
        ...
        send_json_response(message.chat.id, f"حدث خطأ: {str(e)}", False, command="add", reply_to_message_id=message.message_id)
...
def show_friends(message):
    ...
    if not friends_list:
        send_json_response(message.chat.id, "لا يوجد أصدقاء لعرضهم حالياً", False, command="show", reply_to_message_id=message.message_id)
        return
    ...
    try:
        ...
        send_json_response(message.chat.id, f"حدث خطأ في عرض القائمة: {str(e)}", False, command="show", reply_to_message_id=message.message_id)
...
def remove_friend(message):
    ...
    try:
        ...
        if not uid.isdigit():
            send_json_response(message.chat.id, "يجب أن يكون ID رقمًا صحيحًا!", False, command="rm", reply_to_message_id=message.message_id)
            return
        ...
        send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /rm <ID>", False, command="rm", reply_to_message_id=message.message_id)

@bot.message_handler(commands=['delete_all'])
def delete_all_friends(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="delete_all", reply_to_message_id=message.message_id)
        return
        
    friends_list = Show_FrEinds()
    if not friends_list:
        send_json_response(message.chat.id, "لا يوجد أصدقاء لحذفهم حالياً", False, command="delete_all", reply_to_message_id=message.message_id)
        return
        
    bot.send_message(message.chat.id, "⏳ <b>جاري حذف جميع الأصدقاء...</b>", reply_to_message_id=message.message_id)
...
def change_bio(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="bio", reply_to_message_id=message.message_id)
        return
    ...
    try:
        ...
        send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /bio <نص>", False, command="bio", reply_to_message_id=message.message_id)

@bot.message_handler(commands=['stats'])
def show_stats(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="stats", reply_to_message_id=message.message_id)
        return
    ...
    send_json_response(message.chat.id, "إحصائيات البوت", True, stats_data, "stats", message.message_id)
    log_action("STATS_VIEWED", message.from_user.id)

@bot.message_handler(commands=['addowner'])
def add_owner_command(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="addowner", reply_to_message_id=message.message_id)
        return
        
    try:
        new_owner_id = int(message.text.split()[1])
        if new_owner_id in OWNERS:
            send_json_response(message.chat.id, f"المستخدم ({new_owner_id}) موجود بالفعل في قائمة المالكين", True, {"owner_id": new_owner_id}, "addowner", message.message_id)
            return
            
        OWNERS.append(new_owner_id)
        save_owners(OWNERS)
        
        # إضافة المستخدم كمالك مع نقاط غير محدودة
        ...
        send_json_response(message.chat.id, f"تمت إضافة المالك الجديد بنجاح", True, response_data, "addowner", message.message_id)
        log_action("OWNER_ADDED", message.from_user.id, f"New owner: {new_owner_id}")
        
    except (IndexError, ValueError):
        send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /addowner <user_id>", False, command="addowner", reply_to_message_id=message.message_id)
    except Exception as e:
        send_json_response(message.chat.id, f"حدث خطأ: {str(e)}", False, command="addowner", reply_to_message_id=message.message_id)
        log_action("ADD_OWNER_ERROR", message.from_user.id, str(e))

@bot.message_handler(commands=['removeowner'])
def remove_owner_command(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="removeowner", reply_to_message_id=message.message_id)
        return
        
    try:
        owner_id = int(message.text.split()[1])
        if owner_id not in OWNERS:
            send_json_response(message.chat.id, f"المستخدم ({owner_id}) غير موجود في قائمة المالكين", False, {"owner_id": owner_id}, "removeowner", message.message_id)
            return
        ...
        send_json_response(message.chat.id, f"تمت إزالة المالك بنجاح", True, response_data, "removeowner", message.message_id)
        log_action("OWNER_REMOVED", message.from_user.id, f"Removed owner: {owner_id}")
        
    except (IndexError, ValueError):
        send_json_response(message.chat.id, "خطأ في الصيغة. استخدم: /removeowner <user_id>", False, command="removeowner", reply_to_message_id=message.message_id)
    except Exception as e:
        send_json_response(message.chat.id, f"حدث خطأ: {str(e)}", False, command="removeowner", reply_to_message_id=message.message_id)
...
def list_owners_command(message):
    if not is_owner(message.from_user.id):
        send_json_response(message.chat.id, "ليس لديك صلاحية استخدام هذا الأمر", False, command="listowners", reply_to_message_id=message.message_id)
        return
    ...
    send_json_response(message.chat.id, "قائمة المالكين", True, response_data, "listowners", message.message_id)
...
def delete_all_callback(call):
    ...
    friends_list = Show_FrEinds()
    if not friends_list:
        bot.answer_callback_query(call.id, "لا يوجد أصدقاء لحذفهم!")
        return
        
    bot.answer_callback_query(call.id, "جاري حذف جميع الأصدقاء...")
    for fid, fname in friends_list:
        DeLetE_FrEind(fid)
    ...
    # يمكن إضافة رد JSON هنا إذا رغبت
...
if __name__ == "__main__":
    ...
    # إضافة المالكين الافتراضيين إذا لم يكونوا مسجلين
    users = load_data(USERS_FILE)
    for owner_id in OWNERS:
