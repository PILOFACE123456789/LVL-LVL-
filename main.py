...
from sponge import *
from sponge import xSendTeamMsg
from sponge import Auth_Chat
...
[C][B][ffffff]/ou/[team code]
    ...
    [FFD700]━━━━━━━━━━━━[ffffff]\n''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                        ...
                        if self.result:
                            self.Status , self.Expire = self.result
                            self.CliEnts.send(xSEndMsg(f'''aa
    ''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)
                            self.CliEnts.send(xSEndMsg(f'''
aa''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            time.sleep(0.3)		
                            self.CliEnts.send(xSEndMsg(f'''SPONGE''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))	        
                            time.sleep(0.3)
                            self.CliEnts.close() ; self.CliEnts2.close()
                            self.Connect_SerVer(Token , tok , host , port , key , iv , host2 , port2)	   	       		
                        elif False == self.result:
                        ...
                        self.id , self.nm = (self.input_msg[6:].split(" ", 1) if " " in self.input_msg[6:] else [self.input_msg[6:], "LVL_UP"])  
                        ...
                        self.Zx = ChEck_Commande(self.id)
                        if self.ChEck_ReGister and True == self.Zx:
                            self.CliEnts.send(xSEndMsg(f'[b][c][{ArA_CoLor()}]تم تشجيل دخول by neuf-t', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                            ...
                            if '0500' in self.DaTa2.hex()[0:4] and len(self.DaTa2.hex()) > 30:
                                ...
                                for i in range(10):
                                    self.CliEnts.send(xSendTeamMsg(f'[b][c][{ArA_CoLor()}]تم تشجيل دخول by neuf-t',idT , key , iv))
                                    #time.sleep(0.5)
                                    self.CliEnts2.send(ExiT('000000' , key , iv))
                                time.sleep(4)
                                self.CliEnts.close() ; self.CliEnts2.close()
          ...
          RemaininG : {self.res}
           ...
           [90EE90]Dev : LWESS_ANTIBAN\n''', 2 , self.DeCode_CliEnt_Uid , self.DeCode_CliEnt_Uid , key , iv))
                    ...
                    if '/pp/' in self.input_msg[:4]:
                        ...
                        self.id , self.nm = (self.input_msg[4:].split(" ", 1) if " " in self.input_msg[4:] else [self.input_msg[4:], "NEUF - T"])
                        self.Zx = ChEck_Commande(self.id)	
                        ...
                        self.id = parts[0] 
                        self.ghost_name = parts[1] if len(parts) > 1 else "neuf-t"
                        self.Zx = ChEck_Commande(self.id)
                        if True == self.Zx:
                            try:
    ...
    def Guest_GeneRaTe(self , uid , password):
        ...
        try:
            ...
            time.sleep(0.2)
            print(' - Starting NEUF-T Team Freind BoT !')
            print(f' - Uid : {uid}\n - Password : {password}')
...
def StarT_SerVer():
    ...
    FF_CLient('4122233548','046BE245EDAFF9905E48D8FDED4E31AD87A7AA82F57699AC17E93AD9634DBA0D')
  
StarT_SerVer() 
#{"4122233548": "046BE245EDAFF9905E48D8FDED4E31AD87A7AA82F57699AC17E93AD9634DBA0D"}